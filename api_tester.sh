...
#
# Unsloth Studio servers ALWAYS require a bearer token, so set each endpoint's
# "auth" to an Unsloth API key (Edit Endpoint, or the "auth" field in the
# config file). Keys are per-STUDIO_HOME and outlive any single server, so one
# long-lived key works against every port unsloth_manager.py starts.
